...
class NetworkTrainer(object):
    ...
    def load_specific_checkpoint(self, train=False, model_name="model_best.model"):
        if self.fold is None:
            raise RuntimeError("Cannot load best checkpoint if self.fold is None")
        # self.output_folder ="/home1/mksun/experiment/ckpt/TMI/checkpoint/new_data/3d_fullres/Task100_MALB/nnUNetMultiTrainerV2__nnUNetPlansv2.1/fold_4"
        # print(self.output_folder)
        if isfile(join(self.output_folder, model_name)):
            self.load_checkpoint(join(self.output_folder, model_name), train=train)
        else:
            self.print_to_log_file("WARNING! {} does not exist! Cannot load best checkpoint. Falling "
                                   "back to load_latest_checkpoint".format(model_name))
