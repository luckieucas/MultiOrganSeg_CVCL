...
class nnUNetTrainer(NetworkTrainer):
        ...
        with torch.no_grad():
            ...
            if class_num < 5:
                ...
                for tag in self.y_tags[::-1]:
                    ...
                    filter &= (output_seg != self.x_tags.index(tag)+1)
                    class_num-=1
                output_seg[filter] = 0
    ...
    def finish_online_evaluation(self):
        ...
        print(f"total class:{len(global_dc_per_class)}")
        self.wandb.log({"epoch:":self.epoch,"liver":global_dc_per_class[0],
        "mean dice":np.mean(global_dc_per_class),"val loss":self.all_val_losses[-1],
        "train loss":self.all_tr_losses[-1],
        "train intra loss":self.all_tr_intra_losses[-1],
        "train inter loss":self.all_tr_inter_losses[-1]
        })
        # self.wandb.log({"epoch:":self.epoch,"spleen": global_dc_per_class[0],
        # "right_kidney": global_dc_per_class[1], "left kidney": global_dc_per_class[2]
        # ,"live":global_dc_per_class[3],"pancreas":global_dc_per_class[4],
        # "mean dice":np.mean(global_dc_per_class),"val loss":self.all_val_losses[-1],
        # "train loss":self.all_tr_losses[-1],
        # "train intra loss":self.all_tr_intra_losses[-1],
        # "train inter loss":self.all_tr_inter_losses[-1]
        ...
        self.print_to_log_file("Mean Dice:", np.mean(global_dc_per_class))
