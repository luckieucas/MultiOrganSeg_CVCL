...
class DataLoader3DSemi(SlimDataLoaderBase):
    def __init__(self, data, patch_size, batch_size, has_prev_stage=False,
                 oversample_foreground_percent=0.0, memmap_mode="r", pad_mode="edge", pad_kwargs_data=None,
                 pad_sides=None,stride=16,iou_bound=[0.4, 0.8]):
        """
        This is the basic data loader for 3D networks. It uses preprocessed data as produced by my (Fabian) preprocessing.
        You can load the data with load_dataset(folder) where folder is the folder where the npz files are located. If there
        are only npz files present in that folder, the data loader will unpack them on the fly. This may take a while
        ...
        :param oversample_foreground: half the batch will be forced to contain at least some foreground (equal prob for each of the foreground classes)
        """
        super(DataLoader3DSemi, self).__init__(data, batch_size, None)
        if pad_kwargs_data is None:
            pad_kwargs_data = OrderedDict()
        self.pad_kwargs_data = pad_kwargs_data
        self.pad_mode = pad_mode
        ...
        # need_to_pad denotes by how much we need to pad the data so that if we sample a patch of size final_patch_size
        # (which is what the network will get) these patches will also cover the border of the patients
        self.need_to_pad = np.array([0,0,0])
        if pad_sides is not None:
            if not isinstance(pad_sides, np.ndarray):
                pad_sides = np.array(pad_sides)
            self.need_to_pad += pad_sides
    ...
    def generate_train_batch(self):
        ...
        for j, i in enumerate(selected_keys):
            ...
            case_all_data = np.pad(case_all_data[:], ((0, 0),(need_to_pad[0]//2,need_to_pad[0]-need_to_pad[0]//2),
            ...
            case_all_data1 = case_all_data[:, valid_bbox_x_lb1:valid_bbox_x_ub1,
                                          valid_bbox_y_lb1:valid_bbox_y_ub1,
                                          valid_bbox_z_lb1:valid_bbox_z_ub1]
            
            case_all_data2 = case_all_data[:, valid_bbox_x_lb2:valid_bbox_x_ub2,
                                valid_bbox_y_lb2:valid_bbox_y_ub2,
                                valid_bbox_z_lb2:valid_bbox_z_ub2]
            if seg_from_previous_stage is not None:
                seg_from_previous_stage = seg_from_previous_stage[:, valid_bbox_x_lb1:valid_bbox_x_ub1,
                                                                  valid_bbox_y_lb1:valid_bbox_y_ub1,
                                                                  valid_bbox_z_lb1:valid_bbox_z_ub1]

            case_all_data_donly1 = np.pad(case_all_data1[:-1], ((0, 0),
                                                              (-min(0, bbox_x_lb1),
                                                               max(bbox_x_ub1 - shape[0], 0)),
                                                              (-min(0, bbox_y_lb1),
                                                               max(bbox_y_ub1 - shape[1], 0)),
                                                              (-min(0, bbox_z_lb1), max(bbox_z_ub1 - shape[2], 0))),
                                         self.pad_mode, **self.pad_kwargs_data)
            case_all_data_donly2 = np.pad(case_all_data2[:-1], ((0, 0),
                                                    (-min(0, bbox_x_lb2),
                                                    max(bbox_x_ub2 - shape[0], 0)),
                                                    (-min(0, bbox_y_lb2),
                                                    max(bbox_y_ub2 - shape[1], 0)),
                                                    (-min(0, bbox_z_lb2), max(bbox_z_ub2 - shape[2], 0))),
                                self.pad_mode, **self.pad_kwargs_data)

            case_all_data_segonly1 = np.pad(case_all_data1[-1:], ((0, 0),
                                                                (-min(0, bbox_x_lb1),
                                                                 max(bbox_x_ub1 - shape[0], 0)),
                                                                (-min(0, bbox_y_lb1),
                                                                 max(bbox_y_ub1 - shape[1], 0)),
                                                                (-min(0, bbox_z_lb1), max(bbox_z_ub1 - shape[2], 0))),
                                           'constant', **{'constant_values': -1})
            case_all_data_segonly2 = np.pad(case_all_data2[-1:], ((0, 0),
                                                                (-min(0, bbox_x_lb2),
                                                                 max(bbox_x_ub2 - shape[0], 0)),
                                                                (-min(0, bbox_y_lb2),
                                                                 max(bbox_y_ub2 - shape[1], 0)),
            ...
            if seg_from_previous_stage is not None:
                seg_from_previous_stage = np.pad(seg_from_previous_stage, ((0, 0),
                                                                           (-min(0, bbox_x_lb1), max(
                                                                               bbox_x_ub1 - shape[0], 0)),
                                                                           (-min(0, bbox_y_lb1), max(
                                                                               bbox_y_ub1 - shape[1], 0)),
                                                                           (-min(0, bbox_z_lb1), max(bbox_z_ub1 - shape[2], 0))),
                                                 'constant', **{'constant_values': 0})
                case_all_data_segonly1 = np.concatenate(
                    (case_all_data_segonly1, seg_from_previous_stage), 0)
            case_all_data_donly = np.concatenate((case_all_data_donly1, case_all_data_donly2), axis=0)
            case_all_data_segonly = np.concatenate((case_all_data_segonly1, case_all_data_segonly2), axis=0)
            data.append(case_all_data_donly[None])
            seg.append(case_all_data_segonly[None])
            ul1.append(overlap1_ul)
            br1.append(overlap1_br)
            ul2.append(overlap2_ul)
            br2.append(overlap2_br)
        ...
        return {'data': data, 'seg': seg, 'properties': case_properties, 'keys': selected_keys, 'ul1':ul1, 'br1':br1, 'ul2':ul2, 'br2':br2}
...
class DataLoader3DwithTagSemi(DataLoader3DSemi):
    def __init__(self, data, patch_size, batch_size, tags, has_prev_stage=False,
                 oversample_foreground_percent=0.0, memmap_mode="r", pad_mode="edge", pad_kwargs_data=None,
                 pad_sides=None, stride=16, iou_bound=[0.1, 0.9]):
        # init_args = ()
        super().__init__(data, patch_size, batch_size, has_prev_stage,
                                            oversample_foreground_percent, memmap_mode, pad_mode, pad_kwargs_data,
                                            pad_sides, stride, iou_bound)
        self.tags = tags
